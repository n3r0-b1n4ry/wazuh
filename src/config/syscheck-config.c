...
int read_reg(syscheck_config *syscheck, char *entries, int arch, char *tag, const char *restrictfile)
{
    int j;
    char **entry;
    char *tmp_str;
    ...
    for (j = 0; entry[j]; j++) {
        ...
        /* Add new entry */
        dump_syscheck_entry(syscheck, tmp_entry, arch, 1, restrictfile, 0, clean_tag, NULL, -1);
    ...
    for (i = 0; node && node[i]; i++) {
        ...
        /* Get Windows registry */
            ...
            if (strcmp(arch, "both") == 0) {
                if (!(read_reg(syscheck, node[i]->content, ARCH_32BIT, tag, restrictfile) &&
                read_reg(syscheck, node[i]->content, ARCH_64BIT, tag, restrictfile))) {
                    free(tag);
                    os_free(restrictfile);
                    return (OS_INVALID);
                }
                ...
                if (!read_reg(syscheck, node[i]->content, ARCH_64BIT, tag, restrictfile)) {
                    free(tag);
                    os_free(restrictfile);
                    return (OS_INVALID);
                }
